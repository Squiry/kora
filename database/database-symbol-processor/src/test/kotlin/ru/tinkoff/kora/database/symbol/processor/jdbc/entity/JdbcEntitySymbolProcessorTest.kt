...
class JdbcEntitySymbolProcessorTest : AbstractSymbolProcessorTest() {
    ...
    fun testMappersGenerated() {
        compile0(
            listOf(RepositorySymbolProcessorProvider(), JdbcEntitySymbolProcessorProvider()),
            """
        ...
        assertThat(loadClass("\$TestRow_JdbcResultSetMapper").getConstructor().newInstance()).isInstanceOf(
            JdbcResultSetMapper::class.java
        )
        assertThat(loadClass("\$TestRow_ListJdbcResultSetMapper").getConstructor().newInstance()).isInstanceOf(
            JdbcResultSetMapper::class.java
        )
        assertThat(
            loadClass("\$TestRow_JdbcRowMapper").getConstructor().newInstance()
        ).isInstanceOf(JdbcRowMapper::class.java)
    }
}
