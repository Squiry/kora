...
class LogAspectTest : AbstractLogAspectTest() {
    ...
    fun testLogArgWithMapper() {
        compile0(
            listOf(AopSymbolProcessorProvider()), *arrayOf<String>(
                """
                    open class Target {
                        @Log.`in`
        ...
        val aopProxy = TestObject(
            loadClass("\$Target__AopProxy").kotlin,
            new("\$Target__AopProxy", factory, new("MyLogArgMapper"))
        )
