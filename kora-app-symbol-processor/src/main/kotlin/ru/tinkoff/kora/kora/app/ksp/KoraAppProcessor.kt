...
import ru.tinkoff.kora.kora.app.ksp.GraphBuilder.buildGraph
...
import ru.tinkoff.kora.kora.app.ksp.interceptor.ComponentInterceptors.Companion.parseInterceptors
...
import ru.tinkoff.kora.ksp.common.KspCommonUtils.getSymbolsWithAnnotation
...
class KoraAppProcessor(
    ...
    override fun processRound(resolver: Resolver): List<KSAnnotated> {
        ...
        fun deferAnnotated(cn: ClassName, filter: (KSClassDeclaration) -> Boolean = { true }) {
            resolver.getSymbolsWithAnnotation(cn.canonicalName).filterIsInstance<KSClassDeclaration>().filter(filter).forEach { deferred.add(it) }
        }
        deferAnnotated(CommonClassNames.koraApp)
        deferAnnotated(CommonClassNames.module)
        deferAnnotated(CommonClassNames.component) { it.classKind == ClassKind.CLASS && !it.hasAopAnnotations() }
        return deferred
    }

    override fun finish() {
        val components = resolver.getSymbolsWithAnnotation(CommonClassNames.component)
            ...
            .toList()
        if (components.any { !it.validate() }) {
            return
        }
        val modules = resolver.getSymbolsWithAnnotation(CommonClassNames.module)
            .filterIsInstance<KSClassDeclaration>()
            .filter { it.classKind == ClassKind.INTERFACE }
            .toList()
        if (modules.any { !it.validate() }) {
            return
        }
        val applications = resolver.getSymbolsWithAnnotation(CommonClassNames.koraApp)
            .filterIsInstance<KSClassDeclaration>()
            .filter { it.classKind == ClassKind.INTERFACE }
            .toList()
        if (applications.any { !it.validate() }) {
            return
        }

        context(resolver) {
            for (element in applications) {
                val src = parseSrc(element, modules, components)
                val ctx = ProcessingContext(resolver, kspLogger, src)
                for (i in 0 until src.rootSet.size) {
                    ctx.stack.addFirst(GraphBuilder.ResolutionFrame.Root(i))
                }
                context(ctx) {
                    try {
                        val graph = buildGraph()
                        write(element, graph.allModules, graph.components, graph.promisedProxies)
    ...
    private fun parseSrc(declaration: KSClassDeclaration, annotatedModules: List<KSClassDeclaration>, components: List<KSClassDeclaration>): GraphSrc {
        ...
        val annotatedModuleComponentsTmp = annotatedModules
            .flatMap { it.element.getDeclaredFunctions().filter(filterObjectMethods).map { f -> ComponentDeclaration.fromModule(it, f) } }
        ...
        for (componentClass in components) {
            allComponents.add(ComponentDeclaration.fromAnnotated(componentClass))
        }
        allComponents.addAll(mixedInComponents.asSequence().map { ComponentDeclaration.fromModule(rootModule, it) })
        ...
        return GraphSrc(declaration, allModules, components, templateComponents, rootSet)
    }
    ...
    private fun write(declaration: KSClassDeclaration, allModules: List<KSClassDeclaration>, components: List<ResolvedComponent>, promisedProxies: List<TypeSpec>) {
        val interceptors = parseInterceptors(components)
        kspLogger.logging("Found interceptors: $interceptors")
        val applicationImplFile = generateImpl(declaration, allModules)
        val applicationGraphFile = generateApplicationGraph(declaration, allModules, components, interceptors, promisedProxies)
        applicationImplFile.writeTo(environment.codeGenerator, true)
        applicationGraphFile.writeTo(environment.codeGenerator, true)
    }
    ...
    context(resolver: Resolver, ctx: ProcessingContext)
    private fun generateApplicationGraph(
        ...
        promisedProxies: List<TypeSpec>
    ): FileSpec {
        val supplier: KSClassDeclaration = resolver.getClassDeclarationByName(Function0::class.qualifiedName.toString())!!
        val containingFile = declaration.containingFile!!
        val packageName = containingFile.packageName.asString()
        ...
        for (i in graph.indices) {
            ...
            val statement = generateComponentStatement(allModules, interceptors, graph, component)
        ...
        for (promisedProxy in promisedProxies) {
            classBuilder.addType(promisedProxy)
        }
    ...
    private fun generateComponentStatement(allModules: List<KSClassDeclaration>, interceptors: ComponentInterceptors, components: List<ResolvedComponent>, component: ResolvedComponent): CodeBlock {
        ...
        statement.add("),\n")
        statement.add("{ ")
        val dependenciesCode = getDependenciesCode(component)
